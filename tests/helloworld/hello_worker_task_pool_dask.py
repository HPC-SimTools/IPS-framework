...
class HelloWorker(Component):
    ...
    def step(self, timestamp=0.0, **keywords):
        ...
        for i, duration in enumerate(('0.2', '0.4', '0.6')):
            ...
            self.services.add_task('pool', 'meth_' + str(i), 1, cwd, copy.copy(self).myMethod, duration)
            self.services.add_task('pool', 'func_' + str(i), 1, cwd, myFun, duration)

        worker_plugin = DaskWorkerPlugin()

        ret_val = self.services.submit_tasks('pool', use_dask=True, dask_nodes=1, dask_ppw=10, dask_worker_plugin=worker_plugin)
