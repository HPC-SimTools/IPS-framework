...
def test_build_launch_cmd_mpirun():
    ...
    assert cmd == (
        'mpirun n1 2 executable 13 42 : n2 1 executable 13 42',
        {'MPI_DSM_CPULIST': '1,16:4'},
    )
...
def test_build_launch_cmd_srun():
    ...
    assert cmd == (
        'srun -N 1 -n 1 -c 1 --threads-per-core=1 --cpu-bind=cores --gpus-per-task=1 executable 13 42',
        None,
    )
    ...
    assert cmd == (
        'srun -N 1 -n 1 -c 1 --threads-per-core=1 --cpu-bind=cores --gpus-per-task=4 executable 13 42',
        None,
    )
...
def test_init_task_srun(tmpdir):
    ...
    def init_final_task(nproc, tppn, tcpt=0):
        task_id, cmd, _, cores_allocated = tm.init_task(
            ServiceRequestMessage(
                ...
                'c',
                'init_task',
                TaskInit(nproc, 'exe', '/dir', tppn, tcpt, 0, True, True, True, False, [], None),
            )
        )
    ...
    with pytest.raises(ResourceRequestUnequalPartitioningError):
        init_final_task(3, 0)
    ...
    with pytest.raises(BadResourceRequestError):
        init_final_task(5, 0)
    ...
    with pytest.raises(ResourceRequestMismatchError):
        init_final_task(3, 1)
    ...
    assert task_id == 11
    assert cmd == 'srun -N 1 -n 1 -c 2 --threads-per-core=1 --cpu-bind=cores exe '
    fwk.warning.assert_called_once_with(
        'task cpp (4) exceeds maximum possible for 1 procs per node with 2 cores per node, using 2 cpus per proc instead'
    )
    ...
    assert cmd == 'srun -N 2 -n 2 -c 2 --threads-per-core=1 --cpu-bind=cores exe '
    fwk.warning.assert_called_once_with(
        'task cpp (12) exceeds maximum possible for 1 procs per node with 2 cores per node, using 2 cpus per proc instead'
    )
    ...
    (
        ...
        ServiceRequestMessage(
            ...
            'c',
            'init_task',
            TaskInit(1, 'exe', '/dir', 0, 0, 0, True, True, True, False, [], '-extra 1'),
        )
    )
    tm.finish_task(ServiceRequestMessage('id', 'id', 'c', 'finish_task', task_id, None))
    assert task_id == 15
    ...
    task_id, cmd, _, _ = tm.init_task(
        ServiceRequestMessage(
            ...
            'c',
            'init_task',
            TaskInit(4, 'exe', '/dir', 0, 0, 0, True, True, True, False, [], None),
        )
    )

    with pytest.raises(BlockedMessageError):
        tm.init_task(
            ServiceRequestMessage(
                'id',
                'id',
                'c',
                'init_task',
                TaskInit(1, 'exe', '/dir', 0, 0, 0, True, True, True, False, [], None),
            )
        )
    ...
    with pytest.raises(InsufficientResourcesError):
        tm.init_task(
            ServiceRequestMessage(
                ...
                'c',
                'init_task',
                TaskInit(1, 'exe', '/dir', 0, 0, 0, False, True, True, False, [], None),
            )
        )
    ...
    with pytest.raises(GpuResourceRequestMismatchError) as e:
        tm.init_task(
            ServiceRequestMessage(
                'id',
                'id',
                'c',
                'init_task',
                TaskInit(1, 'exe', '/dir', 0, 0, 1, False, True, True, False, [], None),
            )
        )

    assert (
        str(e.value)
        == 'component id requested 1 processes per node with 1 GPUs per process, which is greater than the available 0 GPUS_PER_NODE'
    )
    ...
    (
        ...
        ServiceRequestMessage(
            ...
            'c',
            'init_task',
            TaskInit(2, 'exe', '/dir', 1, 0, 1, False, True, True, False, [], None),
        )
    )
    tm.finish_task(ServiceRequestMessage('id', 'id', 'c', 'finish_task', task_id, None))
    assert task_id == 21
    ...
    (
        ...
        ServiceRequestMessage(
            ...
            'c',
            'init_task',
            TaskInit(2, 'exe', '/dir', 1, 0, 2, False, True, True, False, [], None),
        )
    )
    tm.finish_task(ServiceRequestMessage('id', 'id', 'c', 'finish_task', task_id, None))
    assert task_id == 22
    ...
    (
        ...
        ServiceRequestMessage(
            ...
            'c',
            'init_task',
            TaskInit(2, 'exe', '/dir', 2, 0, 1, False, True, True, False, [], None),
        )
    )
    tm.finish_task(ServiceRequestMessage('id', 'id', 'c', 'finish_task', task_id, None))
    assert task_id == 23
    ...
    with pytest.raises(GpuResourceRequestMismatchError) as e:
        tm.init_task(
            ServiceRequestMessage(
                ...
                'c',
                'init_task',
                TaskInit(2, 'exe', '/dir', 2, 0, 2, False, True, True, False, [], None),
            )
        )

    assert (
        str(e.value)
        == 'component id requested 2 processes per node with 2 GPUs per process, which is greater than the available 2 GPUS_PER_NODE'
    )
...
def test_init_task_pool_srun(tmpdir):
    ...
    retval = init_final_task_pool(
        msg={
            'task0': TaskInit(
                1, 'exe0', '/dir', 0, 0, 0, False, False, True, False, ('arg0',), '-extra 1'
            )
        }
    )
    assert len(retval) == 1
