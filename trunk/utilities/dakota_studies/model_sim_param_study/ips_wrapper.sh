...
# Incorporate the input values from Dakota into model_epa_input.nml
# Note that the INPUT namelist is in the root directory
# and the OUTPUT namelist is in the working directory
../../dakota_utils/createInputNamelist.py $PARAM_FILE model_epa_input.nml "${WORKING_DIR}/model_epa_input.nml"
...
../../dakota_utils/parseIpsConfig.py $WORKING_DIR $IPS_CONFIG_FILE "${WORKING_DIR}/${IPS_CONFIG_FILE}"
...
# Run the test program with the parameters in model_epa_input.nml
$IPS_ROOT/bin/ips --config=$IPS_CONFIG_FILE
