...
class InstanceComponent(Component):
    ...
    def step(self, timestamp: float = 0.0, **keywords):
        ...
        with open('stats.csv', 'w') as f:
            writer = csv.writer(f)
            writer.writerow(['instance', 'executable', 'hostname', 'pid',
                             'core', 'start', 'end'])
            writer.writerow([instance_id, sys.argv[0], run_env['hostname'],
                             run_env['pid'], run_env['core_id'], start, time()])

        try:
            self.services.add_analysis_data_files(['stats.csv'], timestamp)
        except Exception:
            print('did not add data file to portal, check logs')
