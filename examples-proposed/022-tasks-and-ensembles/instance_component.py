...
class InstanceComponent(Component):
    def step(self, timestamp: float = 0.0, **keywords):
        if 'HWLOC_XMLFILE' in os.environ:
            self.services.warning('HWLOC_XMLfile still set!')
        else:
        ...
        self.services.info(
            f'{instance_id}: Launching MPI executable {mpi_executable} in {working_dir}'
        )
        args = [
            '-i',
            instance_id,
            '-s',
            str(self.B),  # arbitrarily using B to specify sleep time
            '-o',
            'stats.csv',
        ]
        cmd = str(mpi_executable) + ' ' + ' '.join(args)
        try:
            run_id = self.services.launch_task(nproc=5, working_dir=working_dir, binary=cmd)
        except Exception:
            self.services.critical(f'{instance_id}: Unable to launch {mpi_executable}')
