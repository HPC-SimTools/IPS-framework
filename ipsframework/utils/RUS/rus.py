    ...
    def __init__(self):
        ...
        try:
            opts, args = getopt.getopt(
                sys.argv[1:], 'r:m:c:l:s:vdfb', ['resource=', 'config=', 'mapfile=', 'log=', 'seed=', 'produce_viz', 'debug', 'failures_on', 'resubmit_on']
            )
    ...
    def go(self):
        ...
        if self.debug:
            ...
            print(
                ...
                self.RM.ppn,
                'ppn,',
                self.fwk_global_time,
                'seconds,',
                (self.RM.nodes * self.RM.ppn * self.fwk_global_time) / 3600.0,
                'CPU hours, ',
                self.simulation_tsteps_completed,
                'simulation timesteps completed',
            )
            if self.failures_on:
                ...
                print(
                    ...
                    self.total_rework,
                    'total rework,',
                    100 * (self.total_rework / float(self.fwk_global_time)),
                    'percent rework',
                    file=self.rlogFile,
                )
