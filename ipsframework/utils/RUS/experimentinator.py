        ...
        for j in range(self.interleaving):
            ...
            if j == 0:
                for nc in range(1, len(y_e[j])):
                    plt.plot([k*4 for k in xn[self.interleaving-1]], y_e[nc]*len(xn[self.interleaving-1]), color_chars[nc % 7]+'--', label='Serial Efficiency %d cores' % (xn[j][nc]))
            ...
            if j == 0:
                plt.plot([k*4 for k in xn[self.interleaving-1]], [temp_x / 1000.0 for temp_x in y_t[0]*len(xn[self.interleaving-1])], color_chars[j % 7]+'--', label='1 Sim')
            else:
