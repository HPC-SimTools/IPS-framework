    ...
    def run(self):
        ...
        if self.nproc > 0:
            ...
            if nodes >= 0:
                ...
                if self.retry == True:
                    if self.sim.retry_limit > 0 and self.curr_retries < self.sim.retry_limit:
                        ...
                        self.fwk.logEvent(
                            ...
                            self.name,
                            'relaunch_task',
                            'relaunched attempt %d on %d processes on %d nodes' % (self.retry, self.using.procs, self.using.nodes),
                        )
                    else:
                    ...
                    self.fwk.logEvent(
                        self.sim.name, self.name, 'start_task', 'started running on %d processes on %d nodes' % (self.using.procs, self.using.nodes)
                    )
            else:
                # -------------------------------------------
                ...
                # --------------------------------
                self.fwk.logEvent(
                    self.sim.name, self.name, 'waiting_on_procs', 'needs %d procs %d memory pproc %d disk pproc' % (self.nproc, self.mem_pproc, self.disk_pproc)
                )
        else:
